...
国能日新·合同智能审查平台当前使用前端本地状态模拟登录：任意非空用户名和密码均可进入系统，用户名被写入 `localStorage`，后端业务接口、PDF、报告、SSE 进度流和质量工作台均未鉴权。任务模型也没有用户归属信息，记录接口会返回全部任务。

本次接入公司 Keycloak 统一认证平台，目标是：

- 前端使用 OIDC Authorization Code + PKCE S256 完成单点登录和退出。
- 后端作为 Resource Server，严格验证 Access Token。
- 使用 Keycloak `sub` 作为用户唯一标识。
- 使用统一应用角色实施接口权限控制。
- 为新创建的合同任务实施本人数据隔离。
- 保持现有 `/api/compare/*` 和 `/api/quality/*` 路由地址不变。
- 不迁移、不补录、不开放历史无归属任务。
...
| 接口或功能 | 后端权限 |
| --- | --- |
| `POST /api/compare` | `agent_admin`、`agent_manager` 或 `agent_user` |
| 本人任务列表 | 任意已认证用户 |
| 本人任务详情、差异、质量摘要、执行状态 | 任意已认证用户 |
| 本人 PDF、报告和 SSE | 任意已认证用户 |
| 取消、重试和审核本人任务 | 三个应用角色之一 |
| 全部新任务及其产物 | `agent_admin` |
| `/api/quality/*` | 仅 `agent_admin` |
| 未来驾驶舱或管理视图 | `agent_admin` 或 `agent_manager` |
| `/health` | 匿名访问 |
...
- 有效 RS256 Token 通过验证。
- 错误签名、过期、错误 issuer、错误 audience 和错误算法返回 `401`。
- `aud` 字符串和数组形式均被正确处理。
- 角色只从固定 Client Role 路径读取。
- 缺少 `sub` 时拒绝认证。
- 三种角色均可创建普通任务。
- 普通用户和管理层只能看到自己的任务。
- 管理员可以访问全部新任务。
- 跨用户访问详情、差异、PDF、报告、SSE、执行状态、取消、重试和审核均返回 `404`。
- 历史无归属任务对所有角色返回 `404`。
- 伪造 owner 或 `reviewed_by` 不生效。
- 未知 `kid` 只触发一次 JWKS 刷新。
- Discovery/JWKS 不可用时失败关闭。
- `/api/quality/*` 仅管理员可访问。
- `/health` 保持匿名可用。

### 11.2 前端测试

- 无会话时只触发一次 Keycloak 跳转。
- `/callback` 完成授权响应处理并恢复原页面。
- 初始化和回调期间不渲染业务页面。
- API、PDF 和 SSE 请求都携带 Bearer Token。
- `401`、`403`、`404` 和 `503` 进入对应处理分支。
- 用户展示名、部门和角色来自统一 Claim。
- 质量工作台入口仅管理员可见。
- 退出调用 OIDC logout 并清理本地状态。
- 不再写入旧认证键。
- 前端构建产物和配置中不存在 Client Secret。
...
1. 未登录访问任意业务页面自动跳转 Keycloak。
2. 登录完成后返回原访问页面。
3. 三种应用角色都能创建合同对比任务。
4. 普通用户和管理层只能查看、下载和操作本人任务。
5. 管理员能访问全部接入后创建的新任务。
6. 用户知道其他人的任务 ID 时也无法访问任何任务子资源。
7. 历史任务不展示且不可直接访问。
8. 质量工作台仅管理员可用，前后端行为一致。
9. Token 过期后可以刷新或重新登录，不出现无限循环。
10. 退出后本地会话清除，并完成 Keycloak 单点退出。
11. 现有 `/api/compare/*` 和 `/api/quality/*` 路由地址保持不变。
12. 后端测试、前端测试和前端生产构建全部通过。
